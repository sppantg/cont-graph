...
(function ( jQuery ) {
  ...
  "Jul", "Aug", "Sept", "Oct", "Nov", "Dec"
          ...
          return "{0} {1}, {2}".formatString( monthNames[date_obj.getMonth()], pretty_date, date_obj.getFullYear()   );
        }
        ...
        var start = function(){
          ...
          loop_html +=  '<text text-anchor="middle" class="wday" dx="-17" dy="22">{0}</text>'.formatString( settings.h_days[0] )+
                        '<text text-anchor="middle" class="wday" dx="-17" dy="48">{0}</text>'.formatString( settings.h_days[1] )+
                        '<text text-anchor="middle" class="wday" dx="-17" dy="74">{0}</text>'.formatString( settings.h_days[2] );

          //Fixed size for now with width= 721 and height = 110
          var wire_html =
            '<svg width="100%" height="auto" viewBox="-10 0 721 110"  class="js-calendar-graph-svg">'+
