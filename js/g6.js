  ...
  "Jul", "Aug", "Sept", "Oct", "Nov", "Dec"
        ...
        var start = function(){
          ...
          loop_html +=  '<text text-anchor="middle" class="wday" dx="-15" dy="22">{0}</text>'.formatString( settings.h_days[0] )+
                        '<text text-anchor="middle" class="wday" dx="-15" dy="48">{0}</text>'.formatString( settings.h_days[1] )+
                        '<text text-anchor="middle" class="wday" dx="-15" dy="74">{0}</text>'.formatString( settings.h_days[2] );

          //Fixed size for now with width= 721 and height = 110
          var wire_html =
            '<svg width="auto" height="170" viewBox="10 0 700 110"  class="js-calendar-graph-svg">'+
        ...
        var mouseEnter = function(evt){
          ...
          var text = "{0} {1} on {2} | Average Class: {3} hours".formatString( count, count_text , date, average_text );
